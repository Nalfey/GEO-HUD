...
If you previously used GEO-HUD 1.6, this version unloads `BCRings` and deletes `Windower4/plugins/BCRings.dll` on load so the old plugin cannot double-draw. You can also remove `load BCRings` from `init.txt` if it is still there.
...
### v1.9.0
* Cardinal Chant circle: `//geohud cardinalchant on` draws a ring under you that follows the target's direction. [Cardinal Chant](https://www.bg-wiki.com/ffxi/Cardinal_Chant) is north = blue magic crit, east = red magic attack, south = green magic accuracy, west = yellow magic burst. In-between bearings blend the two nearest colours.
